"""
Issue #10: POST /api/analyze-ingredients のルーター単体テスト。

image_ai_client.analyze_image はAI担当が実装するスタブ（NotImplementedError）のため、
ここではその戻り値をモックし、ルーター（analyze_ingredients.py）の
リクエスト処理・レスポンス整形のみを検証する。
"""
...
def test_ai_error(monkeypatch):
    async def failing_analyze_image(image_bytes: bytes, mime_type: str):
        raise RuntimeError("AI呼び出し失敗")

    monkeypatch.setattr(analyze_ingredients, "analyze_image", failing_analyze_image)
